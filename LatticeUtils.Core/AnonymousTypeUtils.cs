...
namespace LatticeUtils.Core
{
    ...
    public static class AnonymousTypeUtils
    {
        ...
        /// <param name="isPropertyOrderSignificant">true if the property order must be preserved or false if the properties can be reordered</param>
        /// <returns>the anonymous type</returns>
        public static Type CreateType(IEnumerable<KeyValuePair<string, Type>> typePairs, bool isPropertyOrderSignificant = false)
        {
            return CreateType(typePairs, isPropertyOrderSignificant: isPropertyOrderSignificant, isMutable: false);
        }
        ...
        public static Type CreateMutableType(IEnumerable<KeyValuePair<string, Type>> typePairs, bool isPropertyOrderSignificant = false)
        {
            return CreateType(typePairs, isPropertyOrderSignificant: isPropertyOrderSignificant, isMutable: true);
        }

        private static Type CreateType(IEnumerable<KeyValuePair<string, Type>> typePairs, bool isPropertyOrderSignificant, bool isMutable)
        {
            if (typePairs == null) throw new ArgumentNullException("typePairs");

            // If the property order is not significant, then we'll sort them (so the same set of property names will 
            // result in the same type no matter what order they're passed to us in).
            if (!isPropertyOrderSignificant)
            {
                typePairs = typePairs.OrderBy(pair => pair.Key);
            }

            var propertyNames = typePairs.Select(pair => pair.Key).ToArray();
            var genericTypeDefinition = GetOrCreateGenericTypeDefinition(propertyNames, isMutable: isMutable);

            var propertyTypes = typePairs.Select(pair => pair.Value).ToArray();
            return genericTypeDefinition.MakeGenericType(propertyTypes);
        }
        ...
        private static string GenerateGenericTypeDefinitionName(ICollection<string> propertyNames, bool isMutableType)
        {
            ...
            // * Add the framework name (to avoid any possible name conflict with real anonymous types)
            ...
            // Since we're generating these names at runtime, I'd rather have a deterministic name generation strategy for unit test purposes, 
            // and just so that in general you always be able to predict what type name you'll get for a given set of property names.
